  ...
  <div>
    <div class="card mb-4">
      <div class="card-header">
        <h2 class="card-title">Registro Contabile</h2>
      </div>
    ...
    <div v-if="totali" class="bg-gray-100 rounded-lg shadow p-4 mb-4">
      <div class="flex justify-end">
        <div class="text-right">
          <span class="text-gray-600 mr-2">Totale:</span>
          <span class="text-xl font-bold text-gray-800">{{ formatCurrency(totali.totale) }}</span>
        </div>
      </div>
    </div>
    ...
    <div v-if="loading" class="bg-white rounded-lg shadow p-12 text-center">
      ...
      <p class="text-gray-500">Caricamento registro contabile...</p>
    </div>
    ...
    <div v-else class="bg-white rounded-lg shadow overflow-hidden">
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="divide-y divide-gray-200">
            ...
            <tr v-for="r in registri" :key="`${r.tipo}-${r.id}`" class="hover:bg-gray-50">
              <td class="px-4 py-3 text-sm">{{ formatDate(r.data) }}</td>
              <td class="px-4 py-3 text-sm font-medium">{{ r.numero }}</td>
              <td class="px-4 py-3 text-sm">
                <span :class="r.tipo === 'Fattura' ? 'bg-blue-100 text-blue-800' : 'bg-orange-100 text-orange-800'"
                      class="px-2 py-1 rounded text-xs">
                  {{ r.tipo }}
                </span>
              </td>
              <td class="px-4 py-3 text-sm">{{ r.paziente_cognome }} {{ r.paziente_nome }}</td>
              <td class="px-4 py-3 text-sm text-right">{{ formatCurrency(r.importo) }}</td>
              <td class="px-4 py-3 text-sm text-right">{{ formatCurrency(r.iva) }}</td>
              <td class="px-4 py-3 text-sm text-right font-medium" :class="r.totale < 0 ? 'text-red-600' : ''">
                {{ formatCurrency(r.totale) }}
              </td>
              <td class="px-4 py-3 text-center">
                <span :class="getStatoSdiClass(r.stato_sdi)" class="px-2 py-1 rounded-full text-xs font-medium">
                  {{ r.stato_sdi || 'N/A' }}
                </span>
              </td>
            </tr>
      ...
      <div v-if="pagination.total > 0" class="px-4 py-3 border-t border-gray-200 flex items-center justify-between">
        <p class="text-sm text-gray-500">
          Pagina {{ pagination.page }} di {{ pagination.pages }} ({{ pagination.total }} documenti)
        </p>
...
const loadData = async (page = 1) => {
  loading.value = true
  try {
    let url = `/finanziario/registro-contabile?page=${page}&limit=${pagination.limit}`
    ...
    console.error('Errore caricamento registro contabile:', e)
  } finally {
    loading.value = false
  }
}
