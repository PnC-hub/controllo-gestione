  ...
  <div>
    <div class="card mb-6">
      <div class="card-header flex justify-between items-center">
        <h2 class="card-title">
          <span>Costi Variabili Gestionali Previsionali</span>
          <i class="fas fa-info-circle text-cyan-600 ml-2 cursor-help" title="Questo elenco mostra tutti i costi variabili gestionali dello studio. Sono tutti i costi variabili legati alla gestione ad esempio il pos che mediamente tendono ad incidere in percentuale sull'incasso."></i>
        </h2>
    ...
    <div class="card mb-6">
      <div class="bg-green-100 px-4 py-3">
        <span class="text-green-800">COSTI VARIABILI GESTIONALI TOTALI %:</span>
        <br>
        <span class="font-bold text-green-900 text-lg">{{ formatPercent(stats.totale) }}</span>
      </div>
    </div>
        ...
        <table class="w-full">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Costo Gestionale %</th>
          ...
          <tbody class="divide-y divide-gray-200">
            ...
            <tr v-for="costo in costi" :key="costo.id" class="hover:bg-gray-50">
              ...
              <td class="px-4 py-3 text-sm">{{ formatPercent(costo.prezzo) }}</td>
        ...
        <div class="relative bg-white rounded-lg shadow-xl w-full max-w-md">
          <form @submit.prevent="submitModal">
            ...
            <div class="p-6 space-y-4">
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Prezzo (percentuale %):</label>
                <input
                  type="number"
                  step="0.01"
                  min="0"
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await fetchApi<any>('/costi/previsionali/variabili_gestionali')
    if (res.success) {
...
const submitModal = async () => {
  ...
  try {
    ...
    if (modalAction.value === 'new') {
      res = await fetchApi<any>('/costi/previsionali/variabili_gestionali', {
        method: 'POST',
        body: {
      ...
      res = await fetchApi<any>(`/costi/previsionali/variabili_gestionali/${modalData.value.id}`, {
        method: 'PUT',
        body: {
          descrizione: modalData.value.descrizione,
          prezzo: modalData.value.prezzo,
